...
本 skill 遵循 **Anthropic Agent Skills 开放标准**：只用 shell + read/write 这类每个 agent 都有的能力，不绑特定 agent 的私有 tool（不依赖 ohsql kernel 的 `SshExec`/`SshUpload`/`FlameGraph`，也不依赖 CC 的 `TodoWrite` 等）。这是它能在 Claude Code、OpenAI Codex CLI、ohsql 上原生运行的前提。
...
For non-Claude-Code agents that don't render the `argument-hint` frontmatter
field as UI hints (e.g. OpenAI Codex CLI), this section duplicates the
required parameters in body prose so the LLM can quote them back to the user.

**Capture mode (default)**:
- `host=<ip>` — target host (e.g. `10.0.0.1`)
- `user=<user>` — SSH user (e.g. `root`, `ec2-user`)
- `key=<path>` — SSH private key path (recommended; e.g. `~/.ssh/id_ed25519`)
- `password=<pw>` — SSH password (alternative to key; requires local `sshpass`;
  not supported on OpenAI Codex CLI sandbox)
- `process=<name>` — target process to sample (default: `mongod`)
- `type=oncpu|offcpu` — sampling type (default: `oncpu`)
- `duration=<sec>` — sampling duration in seconds (default: `0.3`)
- `port=<n>` — SSH port (default: `22`)
- `engine=<name>` — KB seed for hotspot interpretation (default: `mongo`)

**Analyze mode** (parse a local SVG):
- `svg=<path>` — local flamegraph.pl SVG file
- `type=oncpu|offcpu` — affects output phrasing (default: `oncpu`)

**Examples**:
```
/cpu-flamegraph host=1.2.3.4 user=root key=~/.ssh/id_ed25519 process=mongod
/cpu-flamegraph host=1.2.3.4 user=root password=secret type=offcpu duration=5
/cpu-flamegraph svg=./flamegraph.svg type=oncpu
```
