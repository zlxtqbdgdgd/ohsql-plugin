...
7. **Phase 4 确认根因 · 默认 KB + NLM 双源 · 单源是降级**:
   - ✅ **默认路径**:KB 阶段命中 → 阶段 2 强制发 NLM query 二次确认 + 求最新建议 → 双源进主表 · 置信度高
   - ✅ KB threshold=NULL(描述性 case)→ NLM 拿初步答案 + references → 单 NLM 源进表 · 置信度中
   - ✅ KB 没覆盖现场现象(如 \$where 烧 CPU)→ 单独发 NLM query 兜底 → 单 NLM 源进表 · 置信度中
   - ✅ NLM 不可用(check / refresh-auth 失败)→ KB-only 降级 · 进表 · 报告头标 "⚠️ NLM 不可用"
   - ❌ **KB 偏离 + NLM 否认 → 不进主表**(移现场观测段)· 不许"反正 KB 命中就强行写进表"
   - ❌ **KB 命中但跳过 NLM 二次确认 → 违规**(本次 spec 改动重点 · LLM 偏见:"我都查到 KB 了再调 NLM 浪费时间")
...
> **首次安装后**:跑 `/perf-kp-sql-setup` 完成 runtime + KB 文件检查与 NotebookLM 注册(零 npm 运行时依赖,无需 `npm install`)。
...
```
Bash(command="ls <候选路径>/scripts/ssh.mjs <候选路径>/data/kb/cases/INDEX.md >/dev/null 2>&1 && echo '<候选路径>'")
```
...
- 仍失败 → 问用户"跳过 NLM 走 KB-only？" · 不阻断主流程
...
进主诊断表的根因 · **默认要求 KB + NLM 双源** — KB 阶段命中给"是不是该 case"· NLM 阶段确认"现在是否仍如此 + 最新修复建议"。两个来源必须能逐条溯回 stdout / Read 输出:

1. **KB 来源**:case_id verbatim 来自 Phase 2.3 Read 出的 KB.md 单 case 字段(DF `likely_causes[]` / Flame `mechanism_quote`)· `[参考N (KB)]` 用 case 的 `source_url` 字段
...
| 场景 | 主表 | 参考来源列 |
|---|---|---|
| KB 偏离 + NLM 确认 | ✅ 进表 · 置信度高 | KB + NLM 双 [参考N] |
| KB threshold=NULL + NLM 给答案 | ✅ 进表 · 置信度中 | 仅 NLM [参考N] |
| KB 偏离 + NLM 不可用(check / relogin 失败) | ✅ 进表 · 置信度中 · 报告头标 "⚠️ NLM 不可用" | 仅 KB [参考N] |
| KB 偏离 + NLM 否认 | ❌ 不进主表 · 移现场观测段 | — |
| KB 没覆盖 + NLM 答得出来 | ✅ 进表 · 置信度中 | 仅 NLM [参考N] |
| KB 没覆盖 + NLM 也答不出 | ❌ 不进主表 · 移现场观测段 | — |

为什么不"KB 命中就够"· 还要加 NLM 二次确认:KB.md 是团队蒸馏快照(每月 / 每季度更新)· 可能落后官方文档最新指导(典型例:THP 在 MongoDB 8.0+ 反过来推荐 always)· 即使 threshold 命中 · 具体修复手段可能已变。NLM 是 Google 检索系统 · 拉的是当前最新文档 references 是真实链接 — KB + NLM 双源 = 团队判断 + 官方现状交叉验证。
...
⚠️ **双阶段强制**:每个候选根因必须先经 KB 阶段判定为"偏离"· 再经 NLM 阶段二次确认 + 求建议。**KB-only 单源不许进主诊断表**(NLM 不可用降级路径除外 · 详见末尾)。
...
- KB 阶段无判定能力 → 直接进阶段 2 · 由 NLM 给初步答案 + 现场指标对照判定

#### 阶段 2 · NLM 二次确认 + 求建议(对每个候选根因强制 · 即使 KB threshold 已命中)

⚠️ **为什么强制**:
- KB.md 是团队蒸馏快照(每月 / 每季度更新)· 可能落后 MongoDB / kernel / kunpeng 官方文档最新指导
- 即使 threshold 命中 · 修复手段(参数值 / 命令 / 配置语法)可能已变(典型例:THP 在 8.0+ 反过来推荐 always)
- NLM references 是真实文档链接 · 跟 KB source_url 互为交叉证据 → 报告"参考来源"列同时挂 KB + NLM 两个 [参考N] · 用户能双源验证

构造 NLM 查询(LLM 拼 · 模板 A:KB 阶段已命中 → 求确认 + 最新建议):
...
> 现场观测:[step.metric_name] = [采集值] (KB 阈值 [threshold] 判定为偏离)。
> KB case [case_id] 描述:[case.symptom_description / abnormal_pattern_quote 节选]。
> KB 给的 mitigation:[likely_cause.reasoning_quote / mitigation_quote 节选]。
>
> 请回答两点:
> 1. 这是否仍是该现象的根因?(是 / 否 / 部分 — 给理由)
> 2. 当前最新的修复建议是什么?(具体命令 / 配置 / 文档链接 · 优先于 KB 节选)

模板 B(KB 阶段无 threshold · NLM 拿初步答案):
...
| KB 阶段 | NLM 阶段 | 结论 | 置信度 | 报告"参考来源"列 |
|---|---|---|---|---|
| 偏离 | 确认 + 推荐与 KB 一致 | 确认根因 | 高 | KB [参考N] + NLM [参考M] |
| 偏离 | 确认 + 推荐与 KB 不同(更新版本) | 确认根因 · 用 NLM 推荐 + 报告标注 "KB 节选可能已过时,以 NLM 引用为准" | 高 | KB [参考N] + NLM [参考M] |
| 偏离 | 否认(不像该 case) | 移到 `## 现场观测(无权威来源)` 段 · **不进主表** | 低 | 不进主表 |
| 偏离 | NLM 不可用(check / refresh-auth 失败) | 进主表 · 但报告头加一行 `⚠️ NLM 不可用 · 本次无 NLM 二次确认 · 请独立验证修复建议` | 中(KB 单源降级)| 仅 KB [参考N] |
...
- KB 推荐 → `[KB]`
- NLM 同意的部分 → `[NLM]`
- KB 与 NLM 不同时 · 写"NLM 推荐(以此为准): xxx [NLM]"<br>"KB 节选(可能过时): yyy [KB]"
...
**标题提取**:
- KB 来源:用 case 的 `source_heading` 字段 · 没有时用 `title` 字段
...
- **根因来源强约束**:Phase 4 每个"确认根因" **默认要求 KB + NLM 双源** — KB 阶段命中后 · 阶段 2 强制发一条 NLM query 二次确认 + 求最新建议 · 综合两者写进主表。NLM 不可用(check / refresh-auth 失败)时降级为 KB-only 单源 · 报告头标 "⚠️ NLM 不可用 · 请独立验证修复建议"。**绝对不许**: 凭训练数据知识写根因 / 编 case_id / 把 KB 多个 case 字段拼一起 / **KB 命中后跳过 NLM 二次确认就进表**(本次 spec 改动重点)。KB 没覆盖的现象(如 \$where 烧 CPU)→ 单独发 NLM query 兜底拿 references → 单 NLM 源进表(置信度中)。NLM 是 Google 检索系统 · references 是真实文档链接 · 跟 KB 双源互为交叉验证。详见 Phase 4 "根因来源强约束" + Phase 4.A 阶段 1/2/3 流程。
