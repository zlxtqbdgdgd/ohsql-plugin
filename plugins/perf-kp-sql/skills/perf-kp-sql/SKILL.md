...
| 5 | 输出报告 | LLM 写 markdown 6 列表 · `md-to-html.mjs` 转 HTML 并机械生成 chat 块 · LLM 透传 stdout | 根因 → 报告文件 (md + html) + chat |
...
| `scripts/md-to-html.mjs <md> <html>` | 报告 markdown → HTML 转换 ([参考N] 脚注 + 抽 `## 诊断结果`/`## 火焰图分析` 包成 stdout 的 `<<<CHAT-OUTPUT … CHAT-OUTPUT>>>` 段) |
...
`md-to-html.mjs` 自动处理 `[参考N]` → `<sup>` 角标 + 脚注卡片(零额外配置)· 并把 chat 段(`## 诊断结果` + 可选 `## 火焰图分析`)从 .md 抽出 · 套上路径头 + 收尾句 · 包在 stdout 的 `<<<CHAT-OUTPUT … CHAT-OUTPUT>>>` 标记里。
...
**步骤 1**:回到 5.3 那次 `md-to-html.mjs` 的 Bash 调用结果 · 在 stdout 里定位 `<<<CHAT-OUTPUT` 与 `CHAT-OUTPUT>>>` 两行 marker。

**步骤 2**:把两行 marker **之间的字面文本**(不含 marker 行本身)直接粘到 chat 输出。**一字不改**:不重排表为 bullet · 不删 `|` 列分隔 · 不加 emoji · 不加叙述前言 · 不加总结收尾。

> 步骤 2 是机械透传 · 不是"基于 stdout 的内容自己写一份"。

**为什么走 stdout 透传 · 不再 Read .md 再复制**:LLM 自己重组文本时 · 永远倾向"叙述总结"+"加 emoji 标记"+"自由发挥结尾" — 这是已经实证压不住的审美偏见。脚本机械生成 chat 块 + LLM 透传 = 从执行路径上消除自由发挥空间。**没有"我写一个简化版"或"竖排更好读"的余地**。
...
- ✅ 透传 5.3 stdout 中 `<<<CHAT-OUTPUT` 与 `CHAT-OUTPUT>>>` 之间的字面文本(不含 marker 行)
- ✅ stdout 字面是 `✓ 报告已生成` + 路径头 + `## 诊断结果` 表 + (可选 `## 火焰图分析` 段) + `详细 [参考N] 引用见 HTML 文件。`
...
- chat 输出报告时 · **透传 5.3 `md-to-html.mjs` stdout 中 `<<<CHAT-OUTPUT … CHAT-OUTPUT>>>` 之间的字面文本** · 不是 emoji bullet 摘要 · 不是字段竖排 · 不是叙述性段落总结。详见 Phase 5.4 输出格式硬约束。
...
| `scripts/md-to-html.mjs` | markdown 报告 → HTML 转换 + chat 块抽取(stdout 含 `<<<CHAT-OUTPUT … CHAT-OUTPUT>>>` 段) |
| `scripts/history.mjs --op load|save` | 历史连接 |
...
- 报告 markdown 字面是 LLM 直出 · HTML 是 `md-to-html.mjs` 机械生成 · chat 块也是 `md-to-html.mjs` 机械抽取(包在 stdout 的 `<<<CHAT-OUTPUT … CHAT-OUTPUT>>>` 标记内) · 三者内容必须 100% 一致(脚注 [参考N] 由 md-to-html 自动转 `<sup>`)
