...
⚠️ **多个候选根因的 NLM query 必须同 message 并行 Bash · 但并发上限 2**:把候选根因的 `notebooklm.mjs --op query` 调用放进**同一个** assistant message 的 Bash content block · 不许"发一条 → 等返回 → 再发下一条"的串行模式。**并发硬上限 2**(超过则分两轮 message 处理):2 并发同 IP 同账号已经接近 Google 反爬 burst 边界 · 5+ 并发会触发限流甚至账号风控。串行 2 条 ≈ 2 × 单 query RTT(60-120s)· 并行 ≈ 1 × 单 query RTT。候选 ≤ 2 → 一轮并行;候选 3-5 → 第一轮 2 并发 + 第二轮其余串/并行(注意 chunk 间留 ≥ 5s 间隔避封号)。
