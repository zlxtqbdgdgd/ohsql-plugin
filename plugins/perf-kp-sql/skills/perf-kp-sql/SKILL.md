...
| 2 | 诊断案例匹配 | LLM 加载 `cases/INDEX.md` 匹配问题现象 → 命中 case_id(≤5 · 内部不暴露) | 问题现象 → 内部 case 列表 |
...
```
Bash(command="node <PLUGIN_ROOT>/scripts/format-chat.mjs --chat /Users/<yourlogin>/.perf-kp-sql/runs/<TS>/report.md --cols <TERMCOLS>")
```
...
- **诊断案例匹配收敛硬约束**(团队规则):候选 ≤ 5 个就停 · 不再追问区分;追问轮数累计 ≤ 5 轮 · 第 5 轮仍 > 5 个就强制带前 5 个进 Phase 3。多 case 并行诊断是标准能力。
