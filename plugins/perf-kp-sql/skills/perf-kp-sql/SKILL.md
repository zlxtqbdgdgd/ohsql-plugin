...
**为什么字面打两遍**:用户看 chat 时以为 skill 在干啥,看 md 报告时也能马上知道这是个什么报告。两个通道目标用户不同 · 都不能省。
...
>
> **旧目录 `reports/` / `flame/` 不再使用**(0.44.0 起单目录归档 · 历史 run 产物原地保留 · 不迁移)。
...
**`--local-svg-out` 说明**(0.44.0 加):wrapper 在 capture.mjs 跑完后会自动从远端 scp 把 SVG 落到本地指定路径(复用 ssh.mjs 已建的 ControlMaster · 不重新 auth)。如果 scp 失败(socket 过期 / 路径异常 / password 模式 ControlMaster 复用不上) · stderr 会打远端 SVG 路径 · LLM 可以手动 `scp` / `mv` 兜底。
...
**性能预期**:阶段 2 每个候选根因发 1 条 NLM query · 单 query timeoutMs=150_000(0.49.x 起)· 平均 RTT 30-60s · 最坏 60-120s。**多个 query 必须同 message 并行**(见阶段 2 标题下分轮表)· 并发硬上限 2 · 候选 ≤ 2 ≈ 单 query RTT 60-120s · 候选 3-5 ≈ ⌈N/2⌉×60-120s + 轮间 5s ≈ 2-6 min。比 4.B 巡检模式(7 min / 93 BP)短得多 · 是合理代价。
...
```
↑ 全没挂 · 这是反例 · 必须每个 `<br>` 子句末尾各挂 1 个 5 标签。0.49.x 起 CLI 不再 lint(`format-chat.mjs --chat` 不会 exit 2),5 标签合规靠 Phase 5.4 self-check 4 条规则兜底。
...
漏挂的拦截在 Phase 5.4 self-check(4 条规则,本节末尾) · 不依赖 CLI lint(0.49.x 起 box-drawing 渲染不跑 lint)。设计参考:`docs/superpowers/specs/2026-05-01-md-report-source-tags-design.md`(标记规范仍生效)。

#### 报告骨架

> **开场白段强制**:报告**最顶部**(标题 `# perf-kp-sql · 性能诊断报告` 之前)**必须**字面复制 chat 通道的开场白文本(详见文档顶部 `# 开场白` 段) · 用 `~~~` 围栏包裹避免跟外层 markdown 内容冲突。该段在 ## 标题之前 · 0.49.x box-drawing CLI 渲染时只透传不解析 · 但**不许省略**。
...
> **0.44.0 路径变更**:从 `~/.perf-kp-sql/reports/perf-kp-sql-<engine>-<TS>.md` 改为 `~/.perf-kp-sql/runs/<TS>/report.md` · 跟 `env.txt` / `collect-os.txt` / `collect-mongo.txt` / `flame.svg` 一起单目录归档。`<engine>` 不再放进文件名(目前只支持 mongo · 没必要重复)。
...
(`<hash>` = sha1(`<host>:<port>:<user>`)[:12] · 跟 ssh.mjs 内部一致。ControlPath 自 0.49.x 已迁
`~/.ssh/perf-kp-sql/`(0700)· 长 HOME 回退 `<tmpdir>/perf-kp-sql-cm-<hash>.sock` · 防 /tmp 共享路径
socket 劫持。ControlMaster 还存活时 scp 不重新 auth)。
...
**lint 现状(0.49.x 起 box-drawing 路径):**
...
- **开场白强制**:skill 一被触发(参数解析 / 历史复用 / 收凭据 之前) · LLM 必须立即向用户打屏 5 步流程预告(详见文档顶部 `# 开场白` 段) · md 报告头部也要字面带这一段。这是 0.44.0 加的 UX 硬约束 · 不许省略 · 不许重写措辞 · 不许加 emoji。
...
- **0.44.0 单目录归档**:本轮 run 所有产物落 `~/.perf-kp-sql/runs/<TS>/` 单目录 — `report.md` / `flame.svg` / `env.txt` / `collect-os.txt` / `collect-mongo.txt`。旧目录 `~/.perf-kp-sql/{reports,flame}/` 已弃用 · 不再写入(SSH command-file 临时文件仍走 `~/.perf-kp-sql/tmp/`)。
...
- 报告文件名:`report.md`,TS = `YYYYMMDD-HHMMSS`(0.44.0 起单目录归档 · 文件名固定 `report.md` · 时间戳进目录路径)
