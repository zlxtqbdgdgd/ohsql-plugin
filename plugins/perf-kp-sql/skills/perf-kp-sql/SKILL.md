...
登录完成后告诉我，或回复"跳过"走仅案例模式。

支持 Chrome/Edge/Brave/Firefox/Safari 等主流浏览器。
```
...
| 2 | ✅ chat 输出 `\|` 字符出现次数**必须 ≥ 40** 次(8 列 × 3 row = 24 cell · 加表头/分隔行 9 × 5 = 45 个 `\|`) | `\|` 数 < 40 → 表已经被压扁成 bullet / 字段竖排 → 违规 |
...
| 4 | ❌ chat 输出**不许同时连续出现** 以下"字段: 值"行模式 · ≥ 3 个就算违规:`根因:` / `风险等级:` / `判断依据:` / `命中案例:` / `建议措施:` / `NotebookLM 返回:` / `置信度:` / `参考文档:` | 出现 ≥ 3 个 → 已把 markdown 8 列表降级成竖排 → 违规 |
...
- 🟡 WARNING · THP=[always] ...
```
把表降级成 emoji bullet · 失去 8 列结构。**违规**。
