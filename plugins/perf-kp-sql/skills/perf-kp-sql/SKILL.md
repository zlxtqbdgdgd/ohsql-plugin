...
| `scripts/md-to-html.mjs <md> <html>` | 报告 markdown → HTML 转换 ([参考N] 脚注 + 抽 `## 诊断结果`/`## 火焰图分析` 包成 stdout 的 `<<<CHAT-OUTPUT … CHAT-OUTPUT>>>` 段 + 同步落盘 `-chat.md` 文件供 Phase 5.4) |
| `scripts/format-chat.mjs --chat <chat.md> [--cols N]` | 按终端宽度对 `-chat.md` 中诊断结果 pipe table 的 cell 内容重新折行(`<br>` 位置动态调整 · 表结构 6 列不变 · 火焰图段原样透传) |
...
cookie 过期时的恢复路径 · **不依赖 Playwright** · 全程走 rookiepy + 系统浏览器：
...
**步骤 1 · 调 format-chat.mjs 拿终端宽度适配后的 chat 文本**:用 Bash 调 `format-chat.mjs` 对 `-chat.md` 中的诊断结果表按终端宽度重新折行（调整 cell 内 `<br>` 位置 · 表结构 6 列不变）。**不要靠 Phase 4 内存里的根因表自己重组**——必须真的发起一次 Bash 调用:

```
Bash(command="COLS=$(tput cols 2>/dev/null || echo ''); node <PLUGIN_ROOT>/scripts/format-chat.mjs --chat /Users/<yourlogin>/.perf-kp-sql/reports/perf-kp-sql-<engine>-<TS>-chat.md $${COLS:+--cols $$COLS}")
```

> `-chat.md` 文件路径就是 5.3 stdout 里 `✓ chat 段已落盘:` 那一行打印的字面值 · 也等于 `.html` 路径把后缀替成 `-chat.md`。
> `format-chat.mjs` 按终端列数动态计算每列字符预算 · 对 cell 内容重新插 `<br>` · 使 pipe table 总宽 ≤ 终端列数。火焰图段和头尾文字原样透传。
...
| `scripts/md-to-html.mjs` | markdown 报告 → HTML 转换 + chat 块抽取(同步落盘 `-chat.md` 文件供 Phase 5.4) |
| `scripts/format-chat.mjs` | 按终端宽度对 `-chat.md` 诊断结果表重新折行(Phase 5.4 调用) |
| `scripts/history.mjs --op load|save` | 历史连接 |
