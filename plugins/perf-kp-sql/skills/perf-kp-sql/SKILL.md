...
6. **报告 `[参考N]` URL 必须 verbatim 来自 CASES.md `source_url` 字段或 NLM `references[].source_id`** · **绝对不许**:
   ...
   - **案例/NLM 都没有时 · 该根因不进主表 · 移到 `## 辅助信息 · 现场观测` 子段**(不许写 `(无案例引用)` 留在主表)
...
```
perf-kp-sql 是一个鲲鹏场景下泛数据库性能诊断 skill，基于全网蒸馏案例集 + NotebookLM 在线知识库诊断，主要流程如下:
...
```
Write(file_path="/tmp/perf-kp-sql-cmd-<TS>.txt", content="<command 字面>")
Bash(command="node <PLUGIN_ROOT>/scripts/ssh.mjs --op exec \
       --host <ip> --user <user> [--privateKeyPath <path> | --password '<pw>'] [--port <n>] \
       --command-file /tmp/perf-kp-sql-cmd-<TS>.txt")
```

`<TS>` 用本轮调用统一的时间戳后缀,用完不必手动删 —— `/tmp/` 是 OS 临时目录,系统会自动清。
...
4. **Phase 4 多源专属**:动作字段固定写"案例库 + NotebookLM 在线知识库" · 把"多源"具体是哪几个源亮出来 · 不挂 `<topic>` / `查证` / `巡检` 等动作。Phase 4 三种 NLM 调用场景共用一个模板。
...
| Phase 2.3 案例库 Read（首条 Read 前 echo 一次，仅作 phase 标记，连贯 task list） | `[2. 诊断案例匹配：202 条案例库索引]` |
...
| Phase 4.* 任意 NLM 调用（单/并行/batch） | `[4. 多源综合诊断：案例库 + NotebookLM 在线知识库]` |
...
```
阶段字面跟 task list 一致 · 动作把"案例库 + NotebookLM 在线知识库"两个源都亮出来 · 不挂耗时 · narration 在 Bash 同 message。
...
```
Bash(command="mkdir -p ~/.perf-kp-sql/runs/<TS>")
```

`<TS>` 是本轮诊断的统一时间戳 `YYYYMMDD-HHMMSS` · Phase 0 解析参数那一刻就钉死 · 后续所有写盘动作(env probe / 采集 / 报告 / 火焰图)全落到 `~/.perf-kp-sql/runs/<TS>/` 这一个目录里。

> **目录用途**(单根 `~/.perf-kp-sql/` · harness-neutral · 跨 Claude Code / Codex / ohsql 一致):
> - `~/.perf-kp-sql/runs/<TS>/` — 单次诊断的所有 run 产物归档(报告 / 采集 / 火焰图)· 见下方 "归档目录布局"
> - `~/.perf-kp-sql/notebooklm.json` — NLM 配置(持久化 · 跨 run 复用)
> - `~/.perf-kp-sql/hosts.json` — SSH/DB 连接历史(mode 0600 · 凭据用户 opt-in 后才存)
> - SSH command-file / NLM query / BP list 等暂存文件落 `/tmp/perf-kp-sql-*-<TS>.{txt,json}` (OS 自动清 · 跟同插件的 ControlMaster socket / askpass 脚本一致)
>
> **多 harness 约束**:user runtime data 路径解析**不接** `${CLAUDE_PLUGIN_DATA}` / `${CODEX_PLUGIN_DATA}` 这类 harness-specific env var · 否则同一用户在不同 harness 下数据分裂。env override 只接 plugin 自己的 `PERF_KP_SQL_HOME`(目录形式)。harness-specific env var 仅用于解析**插件源码 / wrapper 搜索路径**(见 `scripts/capture-flamegraph.mjs` 对 cpu-flamegraph wrapper 的多 harness 候选搜索)。
...
**run 目录之外的辅助路径**:
- `/tmp/perf-kp-sql-*-<TS>.{txt,json}` — SSH command-file / NLM query / BP list / env JSON 暂存(OS 自动清 · 跟 ControlMaster socket / askpass 脚本一致)
- `~/.perf-kp-sql/notebooklm.json` — NLM 配置(持久化 · 跨 run 复用)
- `~/.perf-kp-sql/hosts.json` — SSH/DB 连接历史(mode 0600)
...
```
Write(file_path="/tmp/perf-kp-sql-cmd-<TS>.txt", content="""\
echo '###UNAME###' && uname -a
...
> **路径说明**:
> - 命令文件落 `/tmp/perf-kp-sql-cmd-<TS>.txt`(用完即弃 · 不属于 run 产物)
...
```
Write(file_path="/tmp/perf-kp-sql-env-<TS>.json", content="<JSON of [环境上下文] · 字段名跟 EnvContext 一致>")
Bash(command="node <PLUGIN_ROOT>/scripts/history.mjs --op save \
       --host <ip> --user <user> --port <n> --engine mongo \
       --env \"$(cat /tmp/perf-kp-sql-env-<TS>.json)\"")
```
...
是否把密码也一起保存到 ~/.perf-kp-sql/hosts.json?
...
- ❌ 看到 `nlm login --check` 偶发失败(代理/SSL EOF)· 推断"NLM 整体不可用"(`refresh-auth` 会重启 Chrome 引导用户登录)
...
cookie 过期时 · `refresh-auth` 在子进程里 spawn `nlm login`(CDP 自动弹隔离 Chrome · 用户在弹出的浏览器里完成 Google 登录 · 5 分钟内必须完成 · 整个流程是**单次同步阻塞 Bash 调用**)。LLM 端 timeout 必须 ≥ 330s · 不要复用 `--op check` 的 30s timeout。
...
**调用前先打 chat**(让用户知道一会儿会弹 Chrome):

```
NLM 凭据失效,正在打开 Chrome 让你登录 Google 账号。
完成登录后窗口会自动关,5 分钟内未完成视为失败。
```

**调用**:

```
Bash(command="node <PLUGIN_ROOT>/scripts/notebooklm.mjs --op refresh-auth --json", timeout=330000)
```
...
| `refresh-auth` 返回 | 处理 |
|---|---|
| `{"ok": true, "method": "still_valid", ...}` | ✅ 凭据其实还有效 · 静默继续流程 |
| `{"ok": true, "method": "nlm_login", "chromium": [...]}` | ✅ 用户登录成功 · 静默继续流程 |
| `{"ok": false, "need_chromium_browser": true, "suggestion": "...", "next_steps": [...]}` | 🟡 用户机器没装 Chromium 系浏览器 · 把 `next_steps` 字面打给用户 + 让用户装好后重跑 |
| `{"ok": false, "need_user_action": true, "error": "nlm login 失败", ...}` | 🟡 5 分钟内未完成登录 / 被中断 · 问用户"再试 / 跳 NLM 走 仅案例 / 中止" |

- 成功(`still_valid` / `nlm_login`)→ 公告"NLM 已恢复" → 继续被中断的流程
- `need_user_action` 后用户选再试 → 重跑 `refresh-auth`(同 timeout)
- `need_chromium_browser` 不重试 · 必须先装浏览器
...
| 用户视角 | LLM 内部 |
|---|---|
| 看到:"开始匹配案例" | 内部已收敛 ≤ 3 个 case · 准备 Phase 3 metric |
...
- LLM 看似只问 1-2 个引导问题 · 然后说"开始匹配案例" · 内部所有 case 收敛 / metric 准备都不暴露给用户。
...
```
Write(file_path="/tmp/perf-kp-sql-cmd-<TS>.txt", content="<OS 层合并命令字面>")
Bash(command="node <PLUGIN_ROOT>/scripts/ssh.mjs --op exec \
       --host <ip> --user <user> [--privateKeyPath <path> | --password '<pw>'] [--port <n>] \
       --command-file /tmp/perf-kp-sql-cmd-<TS>.txt \
...
```
Write(file_path="/tmp/perf-kp-sql-cmd-<TS>.txt", content="<MongoDB 层合并命令字面>")
Bash(command="node <PLUGIN_ROOT>/scripts/ssh.mjs --op exec \
       --host <ip> --user <user> [--privateKeyPath <path> | --password '<pw>'] [--port <n>] \
       --command-file /tmp/perf-kp-sql-cmd-<TS>.txt \
...
⚠️ **Read 完后立即进 Phase 4 · 不许在 chat 出任何分析**:Phase 3 头部"采集只采不诊断"约束在 3.A.5 同样生效。Read 拿到 metric 文本只是**预读**,所有 metric→value 解析、案例阈值对比、根因初判**全部留到 Phase 4 阶段 1 案例直判**。3.A.5 之后下一个 chat 文本应该是 Phase 4 的 narration `[4. 多源综合诊断: 案例库 + NotebookLM 在线知识库]` · 不是任何观察 / 描述 / 推断陈述。
...
| 场景 | 主表 | 参考文档列 |
|---|---|---|
| 案例 偏离 + NLM 确认 | ✅ 进表 · 置信度高 | `[参考N]`(URL = 案例 source_url 与 NLM references[].source_id 共同源 · 二选一,优先案例 source_url) |
| 案例 threshold=NULL + NLM 给答案 | ✅ 进表 · 置信度中 | `[参考N]`(URL = NLM references[].source_id) |
| 案例 偏离 + NLM 不可用(check / relogin 失败) | ✅ 进表 · 置信度中 · 报告头标 "⚠️ NLM 不可用" | `[参考N]`(URL = 案例 source_url) |
| 案例 偏离 + NLM 否认 | ❌ 不进主表 · 移辅助信息段 | — |
| 案例 没覆盖 + NLM 答得出来 | ✅ 进表 · 置信度中 | `[参考N]`(URL = NLM references[].source_id) |
| 案例 没覆盖 + NLM 也答不出 | ❌ 不进主表 · 移辅助信息段 | — |
...
- `参考文档` 列写 `[参考N]` · URL = NLM 返回的 `references[].source_id`(verbatim · 不许编)
- 置信度: 中(NLM 兜底)
...
1. **触发 NLM-relogin 流程**(详见 Phase 0.10 #NLM-relogin 段)· 调 `--op refresh-auth`(timeout ≥ 330s) · 子进程 spawn `nlm login` 弹隔离 Chrome 让用户完成 Google 登录 · 单次同步阻塞返回
2. 返回 `method: still_valid` / `nlm_login` → 重新跑被中断的 NLM 调用
...
- NLM references 是真实文档链接 · 跟 案例 source_url 互为交叉证据 → 报告"参考文档"列的 [参考N] 可对应案例 source_url 与 NLM references 任一(URL 内部均可双源溯源验证)

构造 NLM 查询(LLM 拼 · 模板 A:案例 阶段已命中 → 求确认 + 最新建议):

```
Write(file_path="/tmp/perf-kp-sql-nlm-q-<TS>.txt", content="<查询字面>")
```
...
| 案例 阶段 | NLM 阶段 | 结论 | 置信度 | 报告"参考文档"列 |
|---|---|---|---|---|
| 偏离 | 确认 + 推荐与 案例 一致 | 确认根因 | 高 | `[参考N]`(URL 二选一,优先案例 source_url) |
| 偏离 | 确认 + 推荐与 案例 不同(更新版本) | 确认根因 · 用 NLM 推荐 + 在 NotebookLM 返回 cell 标注 "案例节选可能已过时,以 NLM 引用为准" | 高 | `[参考N]`(URL = NLM references[].source_id 优先) |
| 偏离 | 否认(不像该 case) | 移到 `## 辅助信息 · 现场观测` 子段 · **不进主表** | 低 | 不进主表 |
| 偏离 | NLM 不可用(check / refresh-auth 失败) | 进主表 · 但报告头加一行 `⚠️ NLM 不可用 · 本次无 NLM 二次确认 · 请独立验证修复建议` | 中(案例单源降级)| `[参考N]`(URL = 案例 source_url) |
| NULL | NLM 给答案 + references | 进主表 | 中(NLM 单源)| `[参考N]`(URL = NLM references[].source_id) |
| NULL | NLM 不可用 | 不进主表 · 移到辅助信息段 | — | 不进主表 |

报告"建议措施" cell 直接写命令字面 / 参数调整(来自 case `recommendation_quote` 或 NLM answer)· **不再挂 5 标签**。NLM 与案例不同时 · 在"NotebookLM 返回" cell 加一句 "案例节选可能已过时,以 NLM 引用为准" 提示用户。
...
报告骨架(三段式):**综合描述** → **诊断结果**(8 列表 + 紧跟参考链接) → **辅助信息**(火焰图分析 + 现场观测)。

> **设计变更说明** · 旧版 `[CASE]` / `[OBS]` / `[NLM]` / `[LLM]` / `[IDX]` 5 标签 cell-level 来源标记已废弃。新版报告里 cell 内**不挂任何来源标签** · 来源信息通过段落结构(命中案例 / NotebookLM 返回 两列分别承载)隐式表达。`## 来源标记 (debug)` legend 段也一并删除。

#### 表格单元格 `<br>` 换行规范(终端渲染必需)

Claude Code / ohsql 终端宽度通常 80-120 列。8 列 pipe table 如果单行超过终端宽度 · 渲染器会降级成"字段: 值"竖排卡片 · 表结构丢失。

**强制规则**:每个单元格内容**必须用 `<br>` 拆成多个短行** · 每行显示宽度按列差异化:

- **根因**:≤ 16 字符/行 · 用 `<br>` 断
- **风险等级**:单词(`high`/`warning`/`info`)· 不需要 `<br>`
- **判断依据**:每行 ≤ 18 字符 · `实测 ...` / `对照 ...` 各一行
- **命中案例**:每行 ≤ 22 字符 · 案例标题 / 触发条件 各一行
- **建议措施**:命令字面单独一行(可适当长 · 终端会自动折行)。**多步骤建议**(`(1)` / `(2)` / `(3)` 这种枚举)→ **每步前必须加 `<br>` 强制换行** · 不许 3 步挤进一坨长 string · 否则渲染时会按列宽硬切成字符级 wrap · 命令被切在 `({status_` 这种半截字符上不可读
- **NotebookLM 返回**:每行 ≤ 24 字符 · NLM answer 摘要分句
- **置信度**:单字(`高`/`中`/`低`)· 不需要 `<br>`
- **参考文档**:`[参考N]` · 不需要 `<br>`

**示例**(正确):
~~~
| 根因 | 风险等级 | 判断依据 | 命中案例 | 建议措施 | NotebookLM 返回 | 置信度 | 参考文档 |
|---|---|---|---|---|---|---|---|
| WT eviction dirty_target<br>与负载不匹配 | high | 实测 cache 94.7%, dirty 18%<br>对照阈值 95% / target 5% | WT cache 压力 · eviction_dirty_target 调优<br>触发条件 cache>90% 且 dirty>10% | `db.adminCommand({setParameter:1, wiredTigerEngineRuntimeConfig:"eviction_dirty_target=3"})` | dirty_target 默认 5% / trigger 20%<br>写密集应调 2-3% 提前唤醒 eviction | 高 | [参考1] |
~~~

**反例**(错误 · 会触发竖排降级):每个 cell 塞成单行长串 · 不用 `<br>` 拆 · 单行总宽度远超终端列宽 → 渲染器把整张表降级成"字段: 值"竖排卡片(`根因: ...` / `判断依据: ...` 行交错)。
...
本次诊断锁定 N 个性能根因,按风险从高到低:

1. **<根因 1 简称>**:<一句话叙述实测值 + 阈值偏离 + 引发的副作用>。
2. **<根因 2 简称>**:...
3. **<根因 3 简称>**:...

[若有现场观测无背书项 · 一句话引子]:另抓到 <现场现象简述>,建议立即 <处置> 止损。

## 诊断结果

| 根因 | 风险等级 | 判断依据 | 命中案例 | 建议措施 | NotebookLM 返回 | 置信度 | 参考文档 |
|---|---|---|---|---|---|---|---|
| <根因简称(2-3 行 br 拆)> | high/warning/info | 实测 ...<br>对照阈值 ... | <案例标题><br>触发条件 ... | <推荐命令字面> | <NLM answer 摘要(2-3 句)> | 高/中/低 | [参考1] |
| ... | ... | ... | ... | ... | ... | ... | [参考2] |

[参考1] [<标题>](<完整 URL>) — <domain>

[参考2] [<标题>](<完整 URL>) — <domain>
...
- **<现场现象简述>**
  - <具体观测细节(命令 stdout 摘录 / planSummary / runtime / 客户端 IP 等)>
  - 建议措施(凭经验,非权威):<命令> 立即止损 · <排查/改写步骤>
  - 现场证据:
    ```
    <具体的 SSH/Bash 命令输出片段>
    ```
...
- `/Users/<yourlogin>/.perf-kp-sql/runs/<TS>/report.md` — 本报告
- `/Users/<yourlogin>/.perf-kp-sql/runs/<TS>/env.txt` — 环境画像采集原始输出
- `/Users/<yourlogin>/.perf-kp-sql/runs/<TS>/collect-os.txt` — 操作系统层采集原始输出(本次没采则不列)
- `/Users/<yourlogin>/.perf-kp-sql/runs/<TS>/collect-mongo.txt` — MongoDB 层采集原始输出(本次没采则不列)
- `/Users/<yourlogin>/.perf-kp-sql/runs/<TS>/flame.svg` — 火焰图 SVG(本次没采则不列)
~~~

#### 报告骨架硬约束(防 LLM 自由发挥加段)

- ❌ **不许在 `## 诊断结果` 标题下方加 explainer / intro 段**(解释"表怎么读" / "每列是什么" / "高/中置信度怎么算" 等)。用户能直接看懂表头 + 行内容 · 介绍是噪音 · 一段不要。
- ❌ **不许另开 `## 修复步骤` / `## 修复命令` / `## 完整命令` 等独立段重复 cell 内容**。完整修复命令字面必须直接放在「建议措施」cell 里(配合 multi-step `<br>` 拆行规范)· 一行完整可执行 · 不开第二段重抄一遍。
- ❌ **不许在「本次诊断产物文件」段尾加"为什么没采 X / Y"长解释**。没采的文件直接**不列**就行 · 列出来的每条只写"路径 — 是什么文件" 一句话 · 不写"我这次跳过了 X 因为 Y"。
- ❌ **不许加非 spec 段标**(`## 总结` / `## 下一步` / `## 验证方法` / `## 后记` 等)。报告止于「本次诊断产物文件」段。
- ✅ **合规三段式骨架**:`## 综合描述`(2-3 行 prose,一句话每根因)+ `## 诊断结果`(直接 8 列表 · 紧跟 `[参考N]`)+ `## 辅助信息`(`### 火焰图分析` / `### 现场观测` / `### 本次诊断产物文件` 三个固定子段)。这套骨架硬上限 · LLM 不许加 / 不许减 / 不许重排。

#### 列含义说明

| 列 | 内容 | 来源 |
|---|---|---|
| 根因 | 命中根因的简称(2-3 行 `<br>` 拆) | 案例 字段 / NLM 综合 |
| 风险等级 | `high` / `warning` / `info` 字面值 | 案例 `risk_severity` 字段透传 |
| 判断依据 | "实测 X<br>对照阈值 Y" 双行格式 | 实测 = Phase 3 SSH stdout · 对照 = `abnormal_pattern_threshold` / NLM 推荐 |
| 命中案例 | 案例标题 + 触发条件 | 案例 `title` / `summary_quote` + `abnormal_pattern_threshold` |
| 建议措施 | 命令字面或参数调整(单条命令一行) | 案例 `recommendation_quote` / NLM answer |
| NotebookLM 返回 | NLM answer 摘要(2-3 句) | NLM `answer` 字段 |
| 置信度 | `高` / `中` / `低` 单字 | 见 4.A 阶段 3 / 4.B 表(双源 = 高 · 单源 = 中 · 否认 = 不进表) |
| 参考文档 | `[参考N]` 角标 | 紧跟表后的链接列表 |

> **置信度判定逻辑**(双源/单源/否认 → 高/中/不进表)是内部标准 · **报告里不显式写"双源"/"单源"/"案例 + NLM 双源"** · 用户视图只剩 `高/中/低` 单字。

#### `[参考N]` 链接列表格式

每条引用单行,格式:
~~~
[参考N] [<标题>](<完整 URL>) — <domain>
~~~

- **位置**:紧跟在 `## 诊断结果` 表格后(不另起 `## 参考` 标题 · 视为表格脚注)
- 条与条之间空一行(md 段落分隔)
- **不再附加来源标签**(原 `[CASE]` / `[NLM]` 来源类型在新结构里不暴露)
- **domain**:从 URL 取 hostname · 去 `www.` 前缀(例 `mongodb.com` / `kernel.org`)
- **标题提取**:案例来源用 `source_heading` / `title` · NLM 来源用 `references[].title` · 缺则从 URL 路径推断短标题
...
- 加到报告末尾的辅助信息段:
  ...
  ~~~
  (该子段语义"无权威来源,仅供参考"由其在辅助信息段下的位置自然表达,标题不再重复)
- **绝对禁止**:把这种根因混进 `## 诊断结果` 主表 · 即使标 `(无案例引用)` · 也不许进主表

**自检规则**(写 5.2 markdown 表前 LLM 必须自检):
- 主诊断表 `## 诊断结果` 表里每一行的 "参考文档" 列 · `[参考N]` 必须能逐条溯回:
  - Phase 2.3 Read 拿到的某个 case 的 source_url 字段(给出 case_id 在内部记录)· 或
  - NLM batch / single query 返回的某条 reference
- 不能溯源的 → **从主表删除** · **移到 `## 辅助信息 · 现场观测` 子段**(不是改写"参考文档"为"无案例引用"留在主表)
- 表后的 `[参考N]` 链接列表里 · **每个 URL 都必须出现在上面 案例 Read 或 NLM 返回的输出里** · 不许新增

**为什么不能"参考文档 = (无案例引用)"留在主表**:
...
- ❌ "WT 缓存驱逐:bytes in cache=63% · 远低于 95% eviction_trigger · 状态正常" 这种把 Phase 2 候选筛掉过程暴露给用户的内容
...
> `format-chat.mjs` 读 .md 报告 · 提取 `## 诊断结果` 8 列 markdown 表 → 按列宽用 box-drawing 重新渲染(`┌─┬─┐ │ ├─┼─┤ └─┴─┘`)→ 包进 \`\`\` 代码块 · 每个 cell 内的 `<br>` 替换为空格再按显示宽度自动换行(不保留 `<br>` 字面)。综合描述 / 辅助信息 / 表后参考链接段原样透传。

**exit code 语义**(脚本实际行为):

| exit | 触发 | 处置 |
|---|---|---|
| `0` | 正常出 stdout | 走步骤 2 字面复制 |
| `1` | 缺 `--chat` 参数 / 报告路径不存在 | 检查 5.3 是否真的落了盘 · 路径是否正确 |
| `2` | 报告里没找到 `## 诊断结果` 段 | 回 Phase 5.2 补 `## 诊断结果` 8 列表后再调 |

**步骤 2 · 字面复制**:把 Bash stdout 返回的整段文本**一字不改**地复制到 chat 输出 · 包括首尾的 \`\`\` 代码块标记。**字面就是字面**:不重排表为 bullet · 不删 `│` 列分隔 · 不加 emoji · 不加叙述前言 · 不加总结收尾 · 不"挑重点 / 改竖排" / 不"翻译成更友好的话"。
...
| # | 规则 | 失败信号 |
|---|---|---|
| 1 | ✅ chat 输出**必须包含** `## 诊断结果` 标题行 · 紧跟一个 \`\`\` 代码块 · 块内首行以 `┌` 起 + `┐` 收(box-drawing 表格上边框 · `format-chat.mjs render8Cols` 的固定特征) | 缺 `## 诊断结果` 或 \`\`\` 代码块或 `┌────┐` 上边框 → 表被吃掉或改写 → 违规 |
| 2 | ✅ chat 输出 `│`(U+2502 BOX VERT · **不是** `\|` U+007C)字符出现次数**必须 ≥ 30** 次(8 列 × 3 row 的合理保守值,标题/分隔/数据行加一起远超此数) | `│` 数 < 30 → 表已经被压扁成 bullet / 字段竖排 → 违规 |
| 3 | ❌ chat 输出**不许包含** `────`(U+2500 重复 ≥ 4 个)**作为整行 row 分隔线**(典型形态:`────────────────────────────` 独占一行 · 前后是"字段: 值"行)。脚本机械生成的边框 `┌────┐` / `├────┤` / `└────┘`(诊断表)和 `╭────┬─` / `├────┼─` / `╰────┴─`(火焰图段)**不算违规**。判据:`────` 行**不以** `┌` / `├` / `└` / `╭` / `╰` 开头 → 违规 | 出现独占一行的 `────` 分隔线(无 box 边框开头字符)→ 已被改成"字段: 值 + ──── 分隔线"竖排 → 违规 |
| 4 | ❌ chat 输出**不许同时连续出现** 以下"字段: 值"行模式 · ≥ 3 个就算违规:`根因:` / `风险等级:` / `判断依据:` / `命中案例:` / `建议措施:` / `NotebookLM 返回:` / `置信度:` / `参考文档:` | 出现 ≥ 3 个 → 已把 8 列表降级成竖排 → 违规 |
...
```
绕过了 stdout 字面的 box-drawing 表 · 用自己的话叙述。**违规**。

**❌ 反例 2 · 表换 bullet / 字段竖排**:
```
根因: WT eviction dirty_target 与负载不匹配
风险等级: high
判断依据: 实测 cache 94.7%, dirty 18% / 对照阈值 95% / target 5%
命中案例: WT cache 压力 · eviction_dirty_target 调优
建议措施: db.adminCommand({setParameter:1, ...})
NotebookLM 返回: dirty_target 默认 5% / trigger 20% ...
置信度: 高
参考文档: [参考1]
────────────────────────────
根因: ...
```
原 stdout 是 box-drawing 渲染的 ASCII 表(`┌─┬─┐ │ ├─┼─┤ └─┴─┘`)· 这里被改成 `字段: 值` 竖排 + `────` 分隔。**违规** — 把表降级成纯文本 · 终端不再渲染列对齐。
...
- 🟡 WARNING · THP=[always] ...
```
把 box-drawing 表降级成 emoji bullet · 失去 8 列结构。**违规**。
...
```
止损命令已经在 box-drawing 表的"建议措施"列里 · 不该单独再起 code block。**违规** — stdout 里没有这段 · 不要"补"。

#### chat 输出格式硬约束

- ✅ 用 Bash 调 `format-chat.mjs`(Phase 5.4 步骤 1) · 把 stdout 字面复制到 chat 输出
- ✅ stdout 字面是报告全文(综合描述 + box-drawing 8 列表 + 表后参考 + 辅助信息)
- ❌ 不要重排 box-drawing 表为 `字段: 值` 竖排 / bullet 列表
- ❌ 不要 emoji 段标(`⚠️` / `🔴` / `🟡` / `ℹ️`)
...
**ohsql skill-doctor 或 LLM 觉得叙述总结更友好 · 都不许改**。用户给装 perf-kp-sql 就是为了看这个 box-drawing 8 列表 · 不是 emoji 摘要 · 也不是字段竖排文本。
...
- chat 输出报告时 · **调 format-chat.mjs 拿 stdout · 字面复制到 chat** · 不是 emoji bullet 摘要 · 不是字段竖排 · 不是叙述性段落总结 · 复制完后必须跑 self-check 4 条硬规则(`## 诊断结果` + \`\`\` 代码块 + `┌────┐` 上边框必含 / `│` ≥ 30 次 / 不许独占的 `────` 分隔行 / 不许 ≥ 3 个"字段: 值"行)· 失败必须回到步骤 2 重做。详见 Phase 5.4 输出格式硬约束。
...
- 暂存文件(SSH command-file / NLM query / BP list / env JSON)落 `/tmp/perf-kp-sql-*-<TS>.{txt,json}` · OS 自动清 · 不在 `~/.perf-kp-sql/` 下堆积
- **单目录归档**:本轮 run 所有产物落 `~/.perf-kp-sql/runs/<TS>/` 单目录 — `report.md` / `flame.svg` / `env.txt` / `collect-os.txt` / `collect-mongo.txt`
...
| Phase 5 报告里 `$match 后置 + $lookup 缺索引是反模式` | `$match 应该先做 现在放到了 $lookup 后面 + 关联字段缺索引` |
| Phase 5 报告里 `造成笛卡尔积效应` | `每条订单先被 $unwind 拆成多份 · 接着两次没索引的 $lookup 都全表扫 · 文档数被放大几个数量级` |
...
- 报告 .md 是 LLM 直出 · chat 输出由 `format-chat.mjs --chat .md` 按终端宽度 box-drawing 渲染后 LLM 字面复制
