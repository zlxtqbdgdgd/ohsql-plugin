...
6. **报告 `[参考N]` URL 必须 verbatim 来自 KB.md `source_url` 字段或 NLM `references[].source_url`** · **绝对不许**:
...
2. **NLM 命中**:NLM 调用(Phase 4.A 单条 query / Phase 4.B BP batch / Phase 3 现场指标的临时 query 兜底)返回的 `answer` + `references` · `[参考N]` 用 NLM 返回的 `references[].source_url`
...
- `(NLM)` — 来自 NotebookLM 返回的 `references[].source_url`
...
- **`[参考N]` URL 强制溯源**:报告 `参考来源` 列每个 `[参考N]` URL · 必须 verbatim 来自 Phase 2.3 Read 出的 KB.md case `source_url` 字段 · 或 NLM 返回的 `references[].source_url`。**绝对不许** 凭记忆写 URL / 按 URL 命名模式推断 / 凭训练数据知识联想 · 即使 URL "看起来合理"。KB/NLM 都没有时 · 该 row 写 `(无 KB 引用)` 而不是编 URL。详见 Phase 5.2 "URL 强制溯源约束" 段。
