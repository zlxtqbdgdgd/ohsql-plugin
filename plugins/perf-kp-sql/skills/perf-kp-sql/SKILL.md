...
- **Collect** — local shell + `node <PLUGIN_ROOT>/scripts/ssh.mjs --op exec` (内部 spawn 本地 OpenSSH `ssh` · ControlMaster 多路复用 · 密码走 SSH_ASKPASS · key 走 -i)运行 per-engine batch commands on the remote host
...
- **Analyze** — local shell: `node <PLUGIN_ROOT>/scripts/diagnose.mjs --os-file ... --db-file ... --engine <name>`
- **Knowledge base** — read / grep over `<PLUGIN_ROOT>/data/<engine>/` + `data/common/`
- **Flamegraph** — local shell: `node <PLUGIN_ROOT>/scripts/capture-flamegraph.mjs ...` (内部自定位 cpu-flamegraph 插件,跨 harness 兼容)
...
stdout 是字面绝对路径(形如 `/Users/<login>/.ohsql/plugins/cache/perf-kp-sql@0.25.5`)· 整个 skill 流程都用此值替换 `<PLUGIN_ROOT>`。stdout 空时跑 AskUserQuestion 让用户填(选项详见 `perf-kp-sql-setup/SKILL.md` Step 1 fallback)。
...
读 probe 命令:
```
Read(file_path="<PLUGIN_ROOT>/data/collect-cmds.json")
```
...
Bash(command="node <PLUGIN_ROOT>/scripts/ssh.mjs --op exec --host <ip> --user <user> [--privateKeyPath <path> | --password '<pw>'] --port <n> --command-file /Users/<yourlogin>/.perf-kp-sql/tmp/perf-kp-sql-cmd-probe-<TS>.txt --timeout 15000")
```
...
调 probe-parse 解析:
```
Bash(command="node <PLUGIN_ROOT>/scripts/ssh.mjs --op probe-parse --probe-file /Users/<yourlogin>/.perf-kp-sql/tmp/perf-kp-sql-probe-<TS>.txt --hint-engine <engine|skipped>")
```
...
Bash("node <PLUGIN_ROOT>/scripts/ssh.mjs --op exec --host <ip> --user <user> [--privateKeyPath <path> | --password '<pw>'] --port <n> --command-file /Users/<yourlogin>/.perf-kp-sql/tmp/perf-kp-sql-cmd-os-<TS>.txt")
```
...
```
Bash(command="node <PLUGIN_ROOT>/scripts/ssh.mjs --op discover --os-file /Users/<yourlogin>/.perf-kp-sql/tmp/perf-kp-sql-os-<TS>.txt --hint-engine <engine>")
```
...
SSH + DB 都成功后静默调:
```
Bash(command="node <PLUGIN_ROOT>/scripts/history.mjs --op save --host <host> --user <user> --port <ssh_port> --engine <engine>")
```
...
```
Bash(command="node <PLUGIN_ROOT>/scripts/ssh.mjs --op session-close --host <ip> --user <user> --port <n>")
```
...
```
Bash(command="node <PLUGIN_ROOT>/scripts/diagnose.mjs --engine <engine> --summary-only --os-file <os-path> --db-file <db-path> --out-json <diag-json-path>")
```
...
首选 read 目录 + INDEX.md:
```
Read(file_path="<PLUGIN_ROOT>/data/<engine>/INDEX.md")
Read(file_path="<PLUGIN_ROOT>/data/<engine>/<file>.md")
```
...
```
Bash(command="node <PLUGIN_ROOT>/scripts/kb.mjs --op stats --engine <engine>")
```
...
```
node <PLUGIN_ROOT>/scripts/render-html-report.mjs \
...
```
Bash(node <PLUGIN_ROOT>/scripts/kb.mjs --op query --rule-id <rule_id> --engine <engine> --top-k 3)
```
...
```
Bash(command="node <PLUGIN_ROOT>/scripts/render-screen-footer.mjs \
       --from-diagnose <diag-json-path> \
...
```
Bash(command="node <PLUGIN_ROOT>/scripts/notebooklm.mjs --op check --json")
```
...
```
Bash(node <PLUGIN_ROOT>/scripts/kb.mjs --op query --q "<原话>" --engine <engine> --top-k 5)
```
