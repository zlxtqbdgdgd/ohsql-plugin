...
6. **报告 `[参考N]` URL 必须 verbatim 来自 CASES.md `source_url` 字段或 NLM `references[].source_url`** · **绝对不许**:
   ...
   - **案例/NLM 都没有时 · 该根因不进 `## 诊断结果` 主表 · 移到 `## 现场观测(无权威来源)` 独立段**
...
7. **Phase 4 确认根因 · 默认 案例 + NLM 双源 · 单源是降级**:
   ...
   - ✅ NLM 不可用(check / refresh-auth 失败)→ 仅案例 降级 · 进表 · 报告头标 "⚠️ NLM 不可用 · 本次无 NLM 二次确认 · 请独立验证修复建议"
   - ❌ **案例 偏离 + NLM 否认 → 不进主表**(移现场观测段)· 不许"反正 案例 命中就强行写进表"
   - ❌ **案例 命中但跳过 NLM 二次确认 → 违规**(本次 spec 改动重点 · LLM 偏见:"我都查到 案例 了再调 NLM 浪费时间")
...
| 4 | 推断与补充 | 案例 阈值直判 / NotebookLM 兜底 | 采集结果 → 确认根因 |
| 5 | 输出报告 | LLM 写 markdown 6 列表落盘 .md · `format-chat.mjs` 按终端宽度重排表格 cell · LLM 字面复制 stdout | 根因 → 报告文件 (md) + chat |
...
4. **Task 清理**:如果当前对话中已存在上一次 `/perf-kp-sql` 的 task list(典型场景:上次跑到一半失败 · 用户重发命令)· **先把旧 task 全部 mark deleted** · 再从 Phase 0 重新声明新 6-phase task list。不许复用旧 task list 的状态 · 不许在旧 task 上 mark in_progress。
...
**No stopping after selection**: same turn → render banner → declare 6-phase task list → 继续 Phase 0 后续步骤(0.4 banner → 0.6 DB 凭据 → 0.7 SSH env probe → 0.9 解析 → 0.10 NLM 探测)。Phase 0 全部子步完成后才 mark phase 0 completed → mark phase 1 in_progress → 进 Phase 1 对话引导。
...
NLM 是 Phase 4 推断的可信兜底(案例 没覆盖现象时拿真实文档 references)· 提前探一下 · 失败时给用户机会现在重新登录 · 比 Phase 4 才发现失败重新打断流程好。
...
**VM/QEMU 环境过滤**(Phase 0 `[环境上下文]` 中 BIOS Vendor 含 QEMU / lscpu 含 Hypervisor vendor / systemd-detect-virt 非 none):
- **跳过**所有 `collection_method_quote` 含 `perf stat` 且带 `-d` flag 的 case(这些需要硬件 PMU 计数器 · 虚拟化环境不支持)
- 典型被跳过 case:`kunpeng-tlb-miss-page-size-04`(需 dTLB/iTLB miss 率 · QEMU 无硬件 PMU)
- 对用户不提 · 内部静默跳过即可
...
**决策外部化（强制）**: 收敛完成后 · 进 2.3 之前 · 必须 `Write(file_path="~/.perf-kp-sql/tmp/perf-kp-sql-match-<TS>.json")` 写出匹配决策（Write 是 tool call · 自动进 harness transcript · `claude-code-log --detail full` 可查）:

```json
{
  "phase": "case_match", "ts": "<ISO8601>", "user_symptom": "<用户原话>",
  "env_context": {"arch":"aarch64","cpu_vendor":"HiSilicon","bios_vendor":"QEMU","numa_nodes":1,"mongod_version":"7.0.31","deploy_form":"standalone"},
  "index_scanned": "<粗分结果 · 如 cpu-high 13 个候选>",
  "filtered_out": [
    {"case_id": "<id>", "cases_md_line": 55, "title": "<title>", "reason": "<过滤理由>"}
  ],
  "selected": [
    {"case_id": "<id>", "cases_md_line": 3, "title": "<title>", "reason": "<选择理由>"}
  ],
  "convergence": "<收敛描述>"
}
```
...
2. **NLM 来源**:NLM 调用(Phase 4.A 阶段 2 单条 query / Phase 4.B BP batch / Phase 6 追问)返回的 `answer` + `references` · `[参考N (NLM)]` 用 NLM 返回的 `references[].source_url`
...
| 案例 偏离 + NLM 不可用(check / relogin 失败) | ✅ 进表 · 置信度中 · 报告头标 "⚠️ NLM 不可用 · 本次无 NLM 二次确认 · 请独立验证修复建议" | 仅 案例 [参考N] |
...
- `参考来源` 列写 `[参考N]` · URL = NLM 返回的 `references[].source_url`(verbatim · 不许编)
- 置信度: 中(NLM 兜底)
...
- 每个根因都能逐条说出来源:
  ...
  - "现场根因 X · NLM 命中 · 来自 Phase 4.A 单条 query 返回 / Phase 4.X 临时兜底 query 返回 · references[i].source_url=URL"
- 不能溯源的根因 → **不进主诊断表** · 移到 Phase 5.2 报告的 `## 现场观测(无权威来源)` 独立段 · 标"请独立验证"
- 不许 "我训练数据里见过这个问题" 当来源 — 不算
- **特别强调**:Phase 3 现场采到的根因(\$where 烧 CPU / 异常 query 等)· 如果 NLM 不可用 · **必须先走 NLM-relogin 流程**(详见 Phase 0.10 #NLM-relogin) · 不许直接放弃 NLM 兜底然后把根因塞主表。NLM 真的连不上(用户拒绝重登录 / 重登录后仍失败)· 才允许走"现场观测"独立段。**例外**:如果用户在 Phase 0.10 已明确选择"跳过 NLM" · 该选择在整个流程内持续生效(skip-NLM 模式)· Phase 4 不再尝试 relogin · 直接走 仅案例 降级或"现场观测"段。

**为什么 案例 和 NLM 都算可信 · 但 LLM 自己写不算**:
- CASES.md 是团队蒸馏的 cases · 来自权威文档 · 有 source_url 链接验证
- NLM 是 Google 检索 + Gemini 生成 · 答案绑定 references[].source_url 真实文档
...
**决策外部化（强制）**: 阶段 1 判定完 · 进阶段 2 之前 · 必须 `Write(file_path="~/.perf-kp-sql/tmp/perf-kp-sql-eval-<TS>.json")`:

```json
{
  "phase": "threshold_eval", "ts": "<ISO8601>",
  "evaluations": [
    {"case_id": "<id>", "cases_md_line": 3, "title": "<title>", "metric": "<metric>", "observed": "<实测值>", "threshold": "<阈值>", "verdict": "abnormal|normal", "next": "进阶段2|排除"}
  ],
  "discoveries": [
    {"source": "<采集来源>", "finding": "<发现描述>", "matched_case_id": "<id>", "matched_cases_md_line": 2295, "next": "<处置>"}
  ],
  "summary": {"total_evaluated": 4, "abnormal": 1, "normal": 3, "discoveries": 1, "to_nlm_confirm": ["<case_id>"]}
}
```
...
**决策外部化（强制）**: 阶段 3 完成 · 进 Phase 5 之前 · 必须 `Write(file_path="~/.perf-kp-sql/tmp/perf-kp-sql-verdict-<TS>.json")`:

```json
{
  "phase": "final_verdict", "ts": "<ISO8601>",
  "root_causes": [
    {"case_id": "<id>", "cases_md_line": 2295, "title": "<title>", "case_verdict": "abnormal", "nlm_verdict": "confirmed|denied", "confidence": "高|中", "risk": "high|warning", "case_source_url": "<url>", "nlm_source_url": "<url>"}
  ],
  "eliminated": [
    {"case_id": "<id>", "cases_md_line": 1031, "title": "<title>", "reason": "<排除理由>"}
  ],
  "moved_to_observation": [],
  "nlm_status": "available|skip|unavailable"
}
```

**性能预期**:阶段 2 每个候选根因发 1 条 NLM query · 单 query timeout 60s · 平均 RTT 30-60s · 假设最终主表 3-5 个根因 → Phase 4 累计 NLM 时长 1.5-5 分钟。比 4.B 巡检模式(7 min / 93 BP)短得多 · 是合理代价。

### 4.B · best-practice 巡检(全量经 NLM)

**设计书强制**: 对每个 BP 一律喂 NLM 刷新最新推荐(决策 2 d')。NLM 不可用时退化为 仅案例(报告头标 NLM 不可用)。
...
| `[NLM]` | NotebookLM stdout JSON 的 references[].source_url 或答复正文 |
...
**Self-check(写完报告后必须):**
- 表格 `## 诊断结果` 前 5 列(确认的根因 / 判定依据 / 建议措施 / 风险等级 / 置信度)每个 cell 内每个 `<br>` 行末尾是否各挂 1 个标签?(第 6 列「参考来源」豁免 · 只放 `[参考N]`)
- `## 现场观测` 段每个原子事实(一个 bullet · 一个句子 · 一段命令)是否各挂 1 个标签?
- `## 参考` 段每条引用第 1 行末尾是否挂了 `(案例)` 或 `(NLM检索)`?
...
[参考1] WiredTiger Tuning — source.wiredtiger.com (案例)
        https://source.wiredtiger.com/mongodb-6.0/tune_cache.html
[参考2] vm.swappiness 内核参数 — kernel.org (NLM检索)
...
每条引用两行:
- 第 1 行:`[参考N] <标题> — <domain> (<来源类型>)`
- 第 2 行:`        <完整 URL>`(8 空格缩进)

**来源类型标注**（用中文圆括号 · 不用方括号 · 避免被 stripChatTags 剥掉 · 用户可见）:
- `(案例)` — 来自 CASES.md case 的 `source_url` 字段
- `(NLM检索)` — 来自 NotebookLM 返回的 `references[].source_url`
...
2. **NLM 返回的 `references[].source_url` 字面值**(notebooklm.mjs query / query-batch 的 stdout JSON)
...
- 如果 NLM 返回鉴权失败 → 必须走 NLM-relogin 流程(详见 Phase 0.10)· 不许直接放弃（**例外**: 用户在 Phase 0.10 已选 skip-NLM 时 · 直接走"次选"）
...
**报告文件写入方式**:始终用 Write 工具整文件写入 · **不用 Edit 工具做局部修改**。Edit 对 CJK + box-drawing 混合内容有已知编码 bug（U+FFFD 替换字符）· 导致 "Error editing file" 后需全文重写 · 直接用 Write 更可靠。

Phase 5.4 的 `format-chat.mjs` 直接读这个 .md 文件。
...
- **`[参考N]` URL 强制溯源**:报告 `参考来源` 列每个 `[参考N]` URL · 必须 verbatim 来自 Phase 2.3 Read 出的 CASES.md case `source_url` 字段 · 或 NLM 返回的 `references[].source_url`。**绝对不许** 凭记忆写 URL / 按 URL 命名模式推断 / 凭训练数据知识联想 · 即使 URL "看起来合理"。案例/NLM 都没有时 · 该根因不进主表 · 移到 `## 现场观测(无权威来源)` 段。详见 Phase 5.2 "URL 强制溯源约束" 段。
...
- 不道歉 / 不反省 / 不自述内部出错
- NLM 不可用 → Phase 4.B 退化为 仅案例 · 报告头标 "⚠️ NLM 不可用 · 本次无 NLM 二次确认 · 请独立验证修复建议"
...
- 报告文件名:`perf-kp-sql-<engine>-<TS>.md`，TS = `YYYYMMDD-HHMMSS`
