...
### Step 7: 逐域同步 Notebooks + 添加 URL

为了让用户看到进度(三个领域累计 4-8 分钟,单次 setup 中间用户没反馈会以为卡住),agent **必须按下面的 5 步拆开调用,每域之间在 chat 里给一行进度**。

#### Step 7.1: 拉各域 URL 数,告诉用户接下来要做什么

不让 LLM 自己 parse JSON · 让脚本算好直接给数。**不需要传 `--urls-file`** — 脚本读内置 `data/notebooklm-urls.json`:

```
Bash(command="node <PLUGIN_ROOT>/scripts/notebooklm.mjs --op urls-summary --json")
```
...
```
{
  "ok": true,
  "domains": [
    {"domain": "mongo",   "notebook_name": "ohsql-mongo-kb",   "url_count": 33},
    {"domain": "kunpeng", "notebook_name": "ohsql-kunpeng-kb", "url_count": 4},
    {"domain": "os",      "notebook_name": "ohsql-os-kb",      "url_count": 7}
  ],
  "total": 44
}
```

agent 在 chat 里写一行(占位符替换成 url_count):

```
开始按领域同步 notebooks:mongo <n_mongo> 篇 / kunpeng <n_kunpeng> 篇 / os <n_os> 篇,预计 4-8 分钟
```

#### Step 7.2 / 7.3 / 7.4: 三次 `add-domain`,**严格串行**,每次拿到上一次结果再发下一次
...
执行上面的 Bash · DOMAIN=mongo · 等结果。

- ok=true → chat 写(按 added/pending/failed 优先级):
  - `added_count == 0` 且 `total_count > 0`:`✓ mongo 同步完成 — <total_count> 篇文档已存在,无需新增`
  - `pending_count == 0` 时:`✓ mongo 同步完成 — 新加 <added_count>/<total_count> 篇`(若 `failed_count > 0` 追加 `· <failed_count> 篇失败`)
  - `pending_count > 0` 时:`✓ mongo 同步完成 — 新加 <added_count>/<total_count> 篇(<ready_count> 就绪 + <pending_count> 处理中)`(若 `failed_count > 0` 追加 `· <failed_count> 篇失败`)
- ok=false → chat 写:`✗ mongo 同步失败 — <error>`,**仍继续 7.3**(SKILL 的"某域失败不阻塞其它域"约定)

##### Step 7.3: 同步 kunpeng

同上 · DOMAIN=kunpeng · 等 7.2 结果出来再发。

##### Step 7.4: 同步 os

同上 · DOMAIN=os · 等 7.3 结果出来再发。

#### Step 7.5: 渲染最终 banner

三次 `add-domain` 全部跑完后(成功或失败都算跑完)进 Step 8。

### Step 8: 完成 banner

agent 把 Step 7.2/7.3/7.4 收到的三次 `add-domain` 结果汇总,渲染:
