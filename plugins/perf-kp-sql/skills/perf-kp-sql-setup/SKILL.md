...
### Step 5: 启用路径 — 安装 NotebookLM

前置:用户必须先装 `uv`(notebooklm-mcp-cli 走 uv tool install 装到 PATH)。setup 检测到 uv 不在会输出明确 next_steps 让用户装。

setup 启动隔离 Chrome 走 CDP 登录(独立 `--user-data-dir=~/.notebooklm-mcp-cli/chrome-profiles/default`,跟日常 Chrome profile 不冲突,不需要关闭):
...
| 失败 | 处置 |
|---|---|
| 客户机器无 Chromium 系浏览器 | setup Step 2 提前检测 → 提示装 Chrome/Edge/Brave 等任一,或选 disable 走仅案例 |
