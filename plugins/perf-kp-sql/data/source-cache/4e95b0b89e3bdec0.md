...
<!-- fetched_at: 2026-05-03T18:11:12.576Z -->
<!-- html_bytes: 779792 · text_chars: 37395 -->
<!-- used_by_cases: 1 -->
...
`numReads` | The number of documents scanned or index keys examined during the query execution stage. New in version 5.1 . | `COLLSCAN`, `IXSCAN`
...
`usedDisk` | Whether the stage wrote to disk. New in version 5.3 . | `GROUP`

`spills` | The number of times the stage spilled to disk. New in version 8.2 . | `GROUP`, `SORT`

`spilledBytes` | The total amount of data written to disk in bytes. New in version 8.2 . | `GROUP`, `SORT`

`spilledRecords` | The total number of individual records spilled to disk. New in version 8.2 . | `GROUP`, `SORT`

`spilledDataStorageSize` | The total disk space, in bytes, used by the spilled data. New in version 8.2 . | `GROUP`, `SORT`
...
`$sort` | `spills` | long long | The number of times the `$sort` stage spilled to disk.

`$sort` | `spilledBytes` | long long | An estimate of the number of bytes written to disk in the `$sort` stage before compression.

`$sort` | `spilledRecords` | long long | The total number of individual records spilled to disk.

`$sort` | `spilledDataStorageSize` | long long | The total disk space, in bytes, used by the spilled data.

`$group` | `usedDisk` | boolean | Whether the `$group` stage wrote to disk.

`$group` | `spills` | long long | The number of times the `$group` stage spilled to disk.

`$group` | `spilledBytes` | long long | An estimate of the number of bytes written to disk in the `$group` stage before compression.

`$group` | `spilledRecords` | long long | The total number of individual records spilled to disk.

`$group` | `spilledDataStorageSize` | long long | The total disk space, in bytes, used by the spilled data.
