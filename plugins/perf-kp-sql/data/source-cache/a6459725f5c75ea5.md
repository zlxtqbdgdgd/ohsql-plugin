...
<!-- fetched_at: 2026-05-03T18:11:00.578Z -->
<!-- html_bytes: 336587 · text_chars: 27889 -->
<!-- used_by_cases: 3 -->
...
1 | mongod soft nproc 64000 < br > mongod hard nproc 64000 < br > mongod soft nofile 64000 < br > mongod hard nofile 64000
...
1 | vm .dirty_ratio = 15 < br > vm .dirty_background_ratio = 5
...
1 | $ sysctl - a | egrep "vm.dirty.*_ratio" < br > vm .dirty_background_ratio = 5 < br > vm .dirty_ratio = 15
...
1 | $ sysctl vm .swappiness < br > vm .swappiness = 1
...
1 | $ numactl -- interleave = all mongod < options here >
...
1 | $ sudo numastat - p $ ( pidof mongod ) < br > < br > Per - node process memory usage ( in MBs ) for PID 7516 ( mongod ) < br > Node 0 Total < br > -- -- -- -- -- -- -- - -- -- -- -- -- -- -- - < br > Huge 0.00 0.00 < br > Heap 28.53 28.53 < br > Stack 0.20 0.20 < br > Private 7.55 7.55 < br > -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- - -- -- -- -- -- -- -- - < br > Total 36.29 36.29
...
1 | # set deadline scheduler and 16kb read-ahead for /dev/sda<br>ACTION=="add|change", KERNEL=="sda", ATTR{queue/scheduler}="deadline", ATTR{bdi/read_ahead_kb}="16"<br>
...
1 | $ cat / sys / block / sda / queue / scheduler < br > noop [ deadline ] cfq
...
1 | $ sudo blockdev -- getra / dev / sda < br > 32
...
1 | / dev / mapper / data - mongodb / var / lib / mongo ext4 defaults , noatime 0 0
...
1 | $ grep "/var/lib/mongo" / proc / mounts < br > / dev / mapper / data - mongodb / var / lib / mongo ext4 rw , seclabel , noatime , data = ordered 0 0
...
1 | net .core .somaxconn = 4096 < br > net .ipv4 .tcp_fin_timeout = 30 < br > net .ipv4 .tcp_keepalive_intvl = 30 < br > net .ipv4 .tcp_keepalive_time = 120 < br > net .ipv4 .tcp_max_syn_backlog = 4096
...
1 | $ sysctl net .core .somaxconn < br > net .core .somaxconn = 4096
...
1 | $ rpm - ql Percona - Server - MongoDB - 32 - server | grep selinux < br > / etc / selinux / targeted / modules / active / modules / mongod .pp
...
1 | $ sudo getenforce < br > Enforcing < br >
