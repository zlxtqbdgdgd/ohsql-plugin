...
// skills/perf-kp-sql/src/cli-kb.ts
...
function runStats(values) {
  ...
  if (!ALLOWED_STATS_ENGINES.has(engineRaw)) {
    console.error(`invalid engine: '${engineRaw}' \xB7 \u4EC5\u652F\u6301 ${[...ALLOWED_STATS_ENGINES].join("/")}`);
    process.exit(2);
  }
...
async function runQuery(values) {
  if (values.help) {
...
`
  ...
  const opts = {
    ruleId: typeof values["rule-id"] === "string" ? values["rule-id"] : void 0,
    waitClass: typeof values["wait-class"] === "string" ? values["wait-class"] : void 0,
    engine: typeof values.engine === "string" ? values.engine : void 0,
    engineVersion: typeof values["engine-version"] === "string" ? values["engine-version"] : void 0,
    q: typeof values.q === "string" ? values.q : void 0,
    topK: typeof values["top-k"] === "string" ? Number(values["top-k"]) : 5,
    flameFunction: typeof values["flame-function"] === "string" ? values["flame-function"] : void 0,
    contextAncestors: typeof values["context-ancestors"] === "string" ? values["context-ancestors"].split(",").map((s) => s.trim()).filter(Boolean) : void 0,
    moduleName: typeof values.module === "string" ? values.module : void 0
  };
  if (opts.q) {
...
async function main() {
  ...
  if (op === "stats") return runStats(values);
  if (op === "query") return runQuery(values);
  process.stderr.write(`unknown --op: ${op} \xB7 expect: query | stats
`);
  process.exit(2);
}
