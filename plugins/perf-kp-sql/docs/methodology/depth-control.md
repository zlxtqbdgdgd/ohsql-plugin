...
| 指标 | 定义 | **当前真测值**(11 份历史报告) | 目标值 | 自动量化方法 |
|---|---|---|---|---|
| **citation_density** | 每个原子事实带 `[参考N]` 角标的比例 | **0%** ⚠️ | ≥ 95% | format-chat.mjs 已有 lintReport · 复用 atom 切分 · 加 `[参考N]` 检测分支 |
| **mechanism_depth** | 主表"判定依据"列每行能否切出"现场值 / 阈值 / 比较结论"3 个原子 | **18.2%** | ≥ 90% | 扫 cell 内 `<br>` 行数 · ≥ 3 个 `<br>` + 至少 1 个 `[OBS]` + 至少 1 个 `[CASE]/[NLM]` |
| **multi_source_rate** | 主表行有 案例 + NLM 双 `[参考N]` 的比例 | **39.4%** | ≥ 70% | 扫"参考来源"列 · 数 `[参考N]` 出现次数 · ≥ 2 算双源 |
| **action_verifiability** | "建议措施"列含验证命令的比例(后置一句"验证:" + 具体命令)| **0%** ⚠️ | ≥ 80% | 正则匹配 cell 内 `验证:` / `verify:` 关键字 + 后跟 backtick code |

> **真测脚本**:`scripts/measure-depth-metrics.mjs` · 报告:`data/quality-reports/depth-metrics-measured.json`(2026-05-03 跑 11 份历史 .md 报告 · 排除掉 2 份没诊断结果表的)
>
> **真测值跟 v1 估值的差距**:
>
> | metric | v1 估 | v2 真测 | 差距原因 |
> |---|---|---|---|
> | citation_density | 60% | **0%** | metric 设计想"原子内嵌 `[参考N]`" · 当前报告把 `[参考N]` 放"参考来源"独立列 · 没嵌入原子 · 是设计 vs 实施的偏差 · 不是"假引用" |
> | mechanism_depth | 50% | **18.2%** | LLM 估高了 · 当前报告"判定依据"列大部分只有 1-2 个 `<br>` 子行 · 不到 3 个 |
> | multi_source_rate | 30% | **39.4%** | LLM 估**低**了 · 实际比想象好 · 因为很多报告"参考来源"列已有多个 `[参考N]` |
> | action_verifiability | 40% | **0%** | metric 设计想"验证命令后置"格式 · 当前报告"建议措施"列没有"验证:" 关键字 · 是 spec 没要求 · 不是没建议 |
>
> ⚠️ 0% 不代表"案例库没引用 / 没建议" · 只代表"当前报告未实施 v2 的 metric 设计形式"。
> 节后改进:① 报告骨架强制原子内嵌 `[参考N]` · ② 报告骨架强制"建议措施"附 "验证: \`...\`" 行。
