...
2. **`cases/CASES.md` / `best-practice/CASES.md`** — Phase 2.3 / Phase 3 Read 出来的 case 字段(likely_causes / mechanism_quote / abnormal_pattern_threshold / source_url / 诊断命令本身)
...
| `[CASE]`  | cases/CASES.md / best-practice/CASES.md(字段 verbatim) | likely_causes、mechanism_quote、abnormal_pattern_threshold、source_url、诊断命令本身 |
...
- **罕见情形**:一个 `<br>` 行挤得下两个短原子 → 用 ` · ` 分开,各自挂标签
  ```
  cache=94.7% [OBS] · 阈=95% [CASE]<br>dirty=18% [OBS] · target=5% [CASE]
  ```
...
整个报告里只存在一套 `[CASE]` / `[NLM]` 字面,不与正文标签混淆。
...
| `skills/perf-kp-sql/SKILL.md` Phase 5.2 报告骨架 | 改 | 加 legend 段 · 表格示例每个 cell 加标签 · `## 参考` 段 `(KB)/(NLM)` → `[CASE]/[NLM]` |
| `skills/perf-kp-sql/SKILL.md` Phase 5.4 | 改 | format-chat.mjs lint 失败的处置流程("exit 2 → 必须回 5.2 重写") |
| `skills/perf-kp-sql/SKILL.md` `## 参考` 段格式规范小节 | 改 | `(KB)` / `(NLM)` 写法改成 `[CASE]` / `[NLM]` |
